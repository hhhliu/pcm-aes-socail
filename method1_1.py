# -*- coding: utf-8 -*-
"""
Created on Fri Jan 26 15:42:15 2018
...
if __name__ == '__main__':
    ...
    resnet50_tag = ResNet50(include_top=False, weights=None)
    for layer in resnet50_tag.layers:
        layer.trainable=False 
        
    input = Input(shape=input_shape,name = 'image_input')
    output_resnet_conv_tag = resnet50_tag(input)
    x1 = Flatten(name='flatten')(output_resnet_conv_tag)
    x1 = Dense(1000, activation='relu', name='predictions')(x1)
    model_tag = Model(input=input, output=x1)    
    output_model_tag = model_tag(input)
    x11 = Dense(250, activation='relu', name='tag')(output_model_tag)
    model1=Model(input=input,output=x11)
    
    model=Sequential()
    model.add(Merge([model_tag,model_user,model_group],mode='concat'))
    ...
    model_tag=Dense(399, activation='softmax',name='tag')(x)
    model_user=Dense(240, activation='softmax',name='user')(x)
    model_group=Dense(327, activation='softmax',name='group')(x) 
    ...
    #model.compile(optimizer=sgd,loss='binary_crossentropy', metrics=[my_acc])
    ...
    with h5py.File(path_train, 'r') as train_file:
        ...
        Checkpoint = ModelCheckpoint(filepath='/home/liuhuihui/ME/pagerank/model/method1_1_unchange.h5', monitor='val_loss', verbose=1,
